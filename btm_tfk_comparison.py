...
class BTMTFKComparisonApp:
    def __init__(self, btm_folder, tfk_folder, distance_threshold=4.0):
    ...
    def load_data(self):
        """Load BTM and TFK data from their respective folders."""
        ...
        if len(self.btm_data) == 0 or len(self.tfk_data) == 0:
    ...
    def find_matching_points(self):
        """Find matching points between BTM and TFK measurements."""
        ...
        common_wafers = btm_wafers.intersection(tfk_wafers)
        
        print(f"Common wafers: {sorted(common_wafers)}")
        
        if not common_wafers:
            print("No common wafers found between BTM and TFK data!")
            return
        ...
        for wafer_id in common_wafers:
            ...
            distances = cdist(btm_coords, tfk_coords)
            
            # Find matches within threshold
            btm_indices, tfk_indices = np.where(distances <= self.distance_threshold)
            
            for btm_idx, tfk_idx in zip(btm_indices, tfk_indices):
                ...
                matched_record = {
                    ...
                    'Distance_mm': distances[btm_idx, tfk_idx],
                    'Thickness_Delta': btm_row['Thickness'] - tfk_row['Thickness'],  # BTM - TFK
    ...
    def analyze_thickness_differences(self):
        """Analyze thickness differences between BTM and TFK."""
        if len(self.matched_data) == 0:
        ...
        print(f"  Mean thickness delta (BTM - TFK): {self.analysis_results['mean_delta']:.2f} ± {self.analysis_results['std_delta']:.2f} Å")
    ...
    def create_visualizations(self, output_dir="btm_tfk_comparison_results"):
        ...
        # Scatter plot BTM vs TFK
        ax1.scatter(self.matched_data['TFK_Thickness'], self.matched_data['BTM_Thickness'], 
                   alpha=0.6, s=20)
        ...
        ax2.set_xlabel('Thickness Delta (BTM - TFK) [Å]')
        ...
        if len(wafer_list) > 1:
            ...
            ax3.set_ylabel('Thickness Delta (BTM - TFK) [Å]')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'btm_tfk_thickness_comparison.png'), dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def create_spatial_delta_plot(self, ax):
        ...
        avg_x = (self.matched_data['BTM_X_mm'] + self.matched_data['TFK_X_mm']) / 2
        avg_y = (self.matched_data['BTM_Y_mm'] + self.matched_data['TFK_Y_mm']) / 2
        ...
        cbar.set_label('Thickness Delta (BTM - TFK) [Å]')
    
    def create_coordinate_delta_summary(self, output_dir="btm_tfk_comparison_results"):
        ...
        site_data['Avg_X_mm'] = (site_data['BTM_X_mm'] + site_data['TFK_X_mm']) / 2
        site_data['Avg_Y_mm'] = (site_data['BTM_Y_mm'] + site_data['TFK_Y_mm']) / 2
        ...
        coord_summary.columns = ['Count', 'Delta_Mean', 'Delta_Std', 'Delta_Min', 'Delta_Max', 
                                'Avg_Distance', 'BTM_Mean_Thickness', 'TFK_Mean_Thickness', 'Wafer_Example']
        ...
        output_path = os.path.join(output_dir, 'btm_tfk_coordinate_delta_summary.csv')
        coord_summary.to_csv(output_path, index=False)
        print(f"Coordinate delta summary saved to {output_path}")
        
        # Print key statistics
        print(f"\nCoordinate Delta Summary (BTM vs TFK):")
    ...
    def save_results(self, output_dir="btm_tfk_comparison_results"):
        """Save all results to files."""
        os.makedirs(output_dir, exist_ok=True)
        
        # Save matched data
        if len(self.matched_data) > 0:
            self.matched_data.to_csv(os.path.join(output_dir, 'btm_tfk_matched_data.csv'), index=False)
            print(f"Matched data saved to {output_dir}/btm_tfk_matched_data.csv")
            
            # Create coordinate delta summary
            self.create_coordinate_delta_summary(output_dir)
        
        # Save analysis summary
        with open(os.path.join(output_dir, 'btm_tfk_analysis_summary.txt'), 'w') as f:
            f.write("BTM vs TFK Thickness Comparison Analysis\n")
            f.write("="*50 + "\n\n")
            ...
            if self.analysis_results:
                ...
                f.write(f"Mean thickness delta (BTM - TFK): {self.analysis_results['mean_delta']:.3f} ± {self.analysis_results['std_delta']:.3f} Å\n")
        ...
        print(f"Analysis summary saved to {output_dir}/btm_tfk_analysis_summary.txt")
    
    def run_complete_analysis(self):
        """Run the complete BTM vs TFK analysis workflow."""
        print("Starting BTM vs TFK Thickness Comparison Analysis")
        print("="*60)
...
def main():
    ...
    app = BTMTFKComparisonApp(btm_folder, tfk_folder, distance_threshold=4.0)
